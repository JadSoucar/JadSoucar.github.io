...
A useful object, which tracks how each path $$p\in P$$ relates the edges $$\ell\in E$$, is an incidence matrix. The incidence matrix is a binary of matrix $$A\in \{0,1\}^{|P|\times |E|}$$, where $$A_{ij}$$ is $$1$$ if the $$i$$-th path contains edge $$j$$ and $$0$$ otherwise.
